...
Version 7.0. Version history is in `CHANGELOG.md`. Project status and open items are in `STATE.md`.
...
The arm set is symmetric across modalities: a reactively adapted arm at clinical margins and a systematically adapted arm at reduced margin, per modality. The systematically adapted arm at unchanged margins is not carried, following the supervisory decision, since the reference study already characterises it as OAPT-Clinic.

**"Non-adapted" means reactively adapted at clinical margin.** Under the version 7 supervisory decision, an arm whose plan loses target coverage on a repeat image is rescued by an offline replan at unchanged margin and continues; nothing is removed. A non-adapted arm therefore replans on demand, and the two arm families are distinguished by margin and by adaptation frequency rather than by the presence or absence of replanning. The label is kept for continuity with the reference study, which does the same thing, and it must be defined on first use in the manuscript, since a reader will otherwise take it to mean zero replans. Allocator A24 and A30.

**The workflow is chosen at prescription, on the planning CT.** Following the version 6 supervisory decision, modality, adaptation and fractionation are all fixed before the first fraction and none of them is revisited during the course. A rescue is not an exception: it is a deterministic consequence of the acceptance criterion given the anatomy, not a decision, so each arm remains a fully specified prescription-time workflow and the allocation remains a choice among a fixed option set.

**Each patient carries seven options, not eight.** XT-A, PT-NA and PT-A each carry both fractionation schedules. XT-NA carries one, fixed per patient by clinical eligibility under the version 7 supervisory decision, because XT-NA represents the treatment the patient would receive under current practice and current practice chooses the schedule on protocol criteria rather than on ΔNTCP. Allocator A32; open decision 18, now closed.
...
|---------|---------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| XT-NA   | Photon, clinical margins, reactive adaptation only, at the schedule clinical eligibility assigns | Locked baseline. All ΔNTCP values are referred to this arm. Free on both budgets and assignable for every patient, which is what makes the no-harm property structural |
...
| PT-NA   | Proton, clinical robustness settings, reactive adaptation only | Reproduces the NA-Clinic arm of the reference study |
...
**What dropping the unchanged-margin adaptive arms costs.** One thing, stated rather than absorbed. The study cannot decompose the benefit of adaptation from the benefit of the margin reduction it licenses, since the two travel together by construction; the decomposition is available from the reference study for lung and is not re-derived here. The second cost recorded at version 6, that no conservative plan remains when the coverage screen removes the reduced-margin plan on a block, is void at version 7: the fallback is rescue at unchanged margin and it applies to every arm.

**What rescue gives back, and what it does not.** Where a non-adapted arm is rescued at every block it becomes clinical-margin systematic adaptation, and its contrast with the adapted arm of the same modality is then a pure margin contrast. That estimate arrives free, since both compositions already exist, but it is conditional on a subgroup selected for maximal anatomical change and is not a cohort estimate. It is reported with the number of patients it rests on, and where that number is small the count is reported and the estimate is not. The cell that would make the decomposition clean, clinical margin with systematic adaptation, is not added: it would cost one replan per block per patient, and it could never be selected in any case, since it costs what the adapted arm costs and is worth less. Allocator decision 22 therefore stands.

**Arms and strategies are almost the same object.** An arm is a plan configuration. A strategy, in the allocator’s sense, is an arm together with a fractionation scheme. Three of the four arms generate two strategies each and XT-NA generates one, so each patient’s option set holds seven. Version 5 generated several strategies per arm through the adaptation vector; that multiplicity is gone, and with it the growth of the option set with the number of repeat images. Rescue does not restore it, since it changes the dose composed for a strategy rather than adding strategies.
...
**Two version 7 changes to this count.** XT-NA carries one schedule rather than two, which removes one plan. And each rescue is an additional plan: a non-adapted arm whose plan fails the acceptance criterion on a block acquires a replan on that image, which carries forward and is screened again on later blocks. The upper bound is one rescue per block per non-adapted arm, which would bring a non-adapted arm to the plan count of an adapted one at clinical margin; the expected number is much smaller and is not predictable before the data. The realised count is a study output rather than a design parameter, and planning-hour estimates should carry the bound alongside the nominal.

For comparison, the reference study used twelve dose distributions per patient across fourteen patients. Fifteen per patient before rescues is a comparable load, which is what makes a cohort of similar order feasible. Dropping the unchanged-margin adaptive arms removed six plans per patient, a 43 per cent reduction against the symmetric two-level design, and that saving buys cohort size, which was the binding constraint. Since the cohort must in addition be enriched for cases in which the choice of schedule is genuinely in doubt, the budget should still be confirmed against planning-hour availability before the case matrix is fixed.
...
**The plan count is linear in the number of blocks and the strategy count does not depend on it at all.** Each block contributes one adapted plan per adapted arm and, where the screen fires, one rescue plan per non-adapted arm, so planning cost grows linearly in B, while the option set is fixed at seven whatever B is. At version 5 the strategy count was 2^B and the evaluation cost grew with it; with adaptation reduced to a scalar that growth is gone, and what B still governs is the number of dose fields composed per strategy, which is also linear. This is why the number of repeat CTs is not fixed at two by the design, even though two is expected: it is a data-availability parameter, not a design constraint.
...
ΔNTCP ranks strategies on organ dose alone and carries no information about the target. Two conditions are checked before allocation, and at version 7 neither removes a strategy from the option set.

**Target coverage, judged per block, enforced by rescue.** A plan delivered on a changed anatomy can lose target coverage, and if the target has moved away from the driving organ at risk it can lose coverage while its NTCP improves, so a ranking on ΔNTCP alone would prefer the arm that undertreats the tumour. Coverage is a property of a plan delivered on a given anatomy, so the criterion is applied to each block's plan on its own image and not to the accumulated dose, which is permissive in the wrong direction. An arm whose plan fails is **rescued**: it acquires a replan generated on that image at unchanged margin and unchanged setup error, which carries forward and is screened again on later blocks. Nothing is removed. This is the version 7 supervisory decision and it is what the reference study does. The screen applies symmetrically to the photon arms, whose dose is recomputed on the repeat images; screening protons while exempting photons would bias the measured proton advantage downwards. The criterion is the plan acceptance protocol used at treatment planning, and the screen takes a list of criteria of which all must pass. The worst-case variant, also per block, is retained as a sensitivity analysis.

**Rescue is a phenomenon of the non-adapted arms.** An adapted arm's block plan is optimised on the anatomy it is then evaluated on, so nominal coverage holds by construction. The screen is nonetheless run on every arm and its count reported per arm, because that property must be demonstrated rather than asserted and because a non-zero count on an adapted arm means the scripted replanning failed, which is a separate open question. The case where a reduced-margin plan cannot be made robustly acceptable at plan generation is handled upstream: that patient has no adapted arm of that modality.
...
**What actually protects the individual, and why it is now structural.** The screen is not what does the work, and the manuscript should not claim that it is. The allocator imposes no constraint on the sign of ΔNTCP anywhere. Protection comes from the structure of the option set: XT-NA costs nothing on either budget and has ΔNTCP identically zero, so it dominates every strategy of negative benefit, and substituting it both raises the objective and frees capacity. No optimal allocation contains a harmful strategy, whether or not harmful strategies are present in the option set.

Version 6 could make that argument only where XT-NA was assignable, and the coverage screen could remove it. At version 7 the screen rescues XT-NA like any other arm, so every patient holds a free assignable option and the argument holds cohort-wide. The property is a consequence of the formulation rather than an empirical finding, and must be written as such. The count of patients with no free assignable option is retained as a regression check on the construction, expected to be zero, rather than as a result.

**What replaces it as the diagnostic to be produced.** Rescue frequency, by arm, by schedule and by block. It measures how often the standard-of-care workflow itself requires offline replanning, which is a clinically citable number in its own right, and it is the empirical check on whether a non-adapted arm, and in particular a five-fraction course delivered without systematic adaptation, is plausible at all.

**One coupling created by rescue.** A rescue improves the NTCP of the arm it acts on, and XT-NA is the reference arm, so the acceptance criterion moves the zero point of ΔNTCP for the patients it newly rescues. The shift is a per-patient constant applied to all seven of that patient's options, so intra-patient ordering, the step-ratio threshold and pen\* are unaffected and only the level moves, changing which patients clear a referral threshold. It must be stated, because it makes every reported number conditional on the criterion.
...
| 7    | Option-set diagnostics: rescue frequency by arm, schedule and block; assignable strategies of non-positive ΔNTCP; patients with no free assignable option, expected zero; patients for whom an adapted arm could not be generated | Supporting, reporting |
...
| Plan budget | Road 4.3 | Three arms planned under both schedules and XT-NA under one: 15 plans per patient on the assumption of two repeat images per schedule, plus one plan per rescue. If the hypofractionated schedule is adapted per fraction the count rises as before; the granularity is open decision 23 |
...
- **The zero-cost component of the population mean must still be reported separately.** XT-NA is free on both budgets, so any positive utility carried on it enters the population mean without consuming capacity, and that component is not a result about capacity. What changed at version 7 is that a patient holds one XT-NA rather than two: the schedule is fixed by clinical eligibility, so there is no competition between a free standard-schedule arm and a free hypofractionated one, and open decision 18 closes. Allocator A32. The sign of the utility of a hypofractionated photon arm relative to a standard-schedule one remains unknown and organ-dependent, and it now sets how far the numeraire moves for the patients whose XT-NA is hypofractionated rather than whether an option is dominated.

- **The reference arm depends on the acceptance criterion.** A rescue improves the NTCP of XT-NA for the patients it acts on, so the zero point of their ΔNTCP moves with the criterion. The shift is a per-patient constant across all their options, so orderings and the step-ratio threshold are unaffected and only the level moves. Introduced at version 7 with the rescue mechanism. Allocator 8.2 and open decision 7b.
...
| Aspect | Where | Statement |
| --- | --- | --- |
| Position in the strategy tuple | 4 | s = (modality, adaptation, fractionation, technique). Robustness is determined by the arm, so each (modality, fractionation) group holds two options; with the XT-NA schedule fixed exogenously by clinical eligibility, each patient holds seven. Allocator A32 |
...
**Open items touching fractionation.** Open decision 19, the anatomical site, sits upstream of everything else on the axis. Open decision 10, endpoint selection constrained to models admitting a dose-per-fraction correction, blocks the axis and is itself gated on 19. Open decisions 20, 21 and 23 each touch the axis without blocking it. Decision 18 closed at version 7.
...
| Plan budget | Road 4.3, extractor 15 | Three arms are planned under both schedules and XT-NA under one, so the plan count is linear in the number of schedules for the arms that carry a choice. If the hypofractionated schedule is adapted per fraction, an adapted arm requires six plans rather than three. Rescue plans are additional and their number is not known before the screen has run, so the extractor must allow a variable plan count per (patient, arm). This is the binding cost of the fractionation axis and it is not an extraction cost |
