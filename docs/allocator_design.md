...
**Under version 6 the LP-dominance question has exactly one instance per patient per scheme.** The photon chain holds one rung above the base and cannot be LP-dominated. The proton chain holds two, so the only rung that can fall below the hull is PT-NA, and it does so when the price-efficient route into the proton chain is to enter directly at PT-A. That single question is what Section 6.5 now answers, and the count of patients for whom PT-NA falls below the hull is the reportable form of it.
...
**P2a and P2b nearly coincide under version 6.** With two rungs on the proton chain, the only way the best available upgrade differs from the next rung is when a patient is moved directly from the photon base to PT-A, skipping PT-NA. That is precisely the case in which PT-NA falls below the hull, so the separation between the two heuristics collapses onto the same single question as the LP-dominance count. The two policies are retained, because the case is real and its frequency is a reportable quantity, but the design should not promise that the choice of ranking statistic carries much weight in this configuration.
...
| ID  | Claim                                                                                                                                                                                         | Status       |
|-----|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| T7  | Each multiplier equals the finite difference of optimal utility with respect to its own budget, holding the other fixed. This is T4 stated per resource                                       | To implement |
| T8  | At C_XT = 0 the two-resource solve reproduces the version 4 single-resource result exactly, and therefore still reproduces the reference study through T1                                     | To implement |
| T9  | As C_XT grows beyond the cohort’s total photon adaptation demand, λ_XT falls to zero and every patient not receiving protons holds XT-A, recovering the version 4 free-photon-adaptation case | To implement |
...
Three claims follow from the version 6 option set and are added here.

| ID  | Claim                                                                                                                                                                                                                                                    | Status       |
|-----|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------|
| T12 | No selected strategy varies modality, adaptation or fractionation across blocks. Modality, adaptation and schedule are scalar fields of the strategy record and a mixed course is not representable rather than merely disallowed                        | To implement |
| T13 | With the option set restricted to one scheme and Δτ_XT set so that C_XT does not bind, the per-course occupancy of Section 9 reproduces the version 5 per-block occupancy on a fully adapted arm, and the reference-study ladder through T1 is unchanged | To implement |
| T14 | The number of patients for whom PT-NA falls below the upper hull of their proton chain, augmented with the photon outside option, equals the number for whom Δτ_PT is below the closed-form threshold of Section 6.5, to solver tolerance                | To implement |

T14 is the test that keeps the closed form honest, since Section 6.5 is now a statement about one rung rather than about a family of intermediate counts and can be checked patient by patient.
...
The mean-field constraint and a peak-occupancy constraint describe **different** feasible sets, so the multipliers they produce differ non-trivially and their gap is informative.

A patient adapted in some blocks and not others occupies a short session on some days and a long one on others but enters the mean-field constraint at the course average. This is exact when we consider steady state with staggered starts, because patients at different points in their courses are mixed on any given day and the adapted blocks of different patients do not align. It fails if starts are synchronized, in which case demand is a step function and the peak day exceeds the mean.

The assumption is checked by re-solving with **peak occupancy** in place of the course average, counting any patient adapted in at least one block at the adapted session length for the whole course. This is a conservative bound and costs one additional solve. If the allocation and λ are stable between the two, assumption A2 is not load-bearing; if they are not, the difference is itself a result about the sensitivity of capacity-limited allocation to scheduling regularity.

A full time-stepping simulation would be a stronger test, but it would require operational data on machine downtime, delays and irregular starts that are not available for this cohort. Constructing that variability rather than measuring it would produce a result about the assumed distribution rather than about the clinic. It is left as future work.
...
| A2  | Course-averaged occupancy is a valid conversion from per-fraction time to daily load                                                                                        | Exact in steady state with staggered starts; checked by a peak-occupancy re-solve                                                                                        | Fails for synchronized starts and small cohorts                                                                                                                                                                       |
...
Additional item: whether patients displaced to photons remain in the denominator of the cohort mean. The reference study divides by 14 throughout, so displaced patients are retained and contribute their XT NTCP. This should be preserved for comparability but stated, since it makes the mean an intention-to-treat quantity over the referred population rather than over the treated one.

**Items opened at version 6.**

| ID  | Question                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Blocks                                                                                                   |
|-----|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| 18  | **How to treat a free hypofractionated photon arm.** XT-NA under the hypofractionated schedule costs nothing on either budget, so if its utility is positive for a patient it is selected at no capacity cost. Four treatments were identified and none is adopted here, since the choice is for supervision. (A) An eligibility threshold ε on the gain of the hypofractionated arm over the same arm at standard fractionation, applied before the solve as a filter on the option set. (B) A uniform utility penalty π applied to every hypofractionated option, which sits inside the objective and reads as the price of a schedule change, reportable beside λ_PT and λ_XT. (C) A third budget row for photon stereotactic delivery capacity, which removes the anomaly by pricing a resource that is genuinely scarce rather than by penalising a choice. (D) Exogenous clinical eligibility, in which the hypofractionated arms exist only for patients meeting the protocol criteria for the indication. The trade-offs are recorded in Section 12.1 | The reporting split between the zero-cost and the capacity-constrained components of the population mean |
...
| 20  | **Per-replan cost accounting as a sensitivity bound.** A16 and A19 charge Δτ to every fraction of an adapted block, which overstates the replanning effort by the number of fractions per block and therefore by a different factor for each schedule. A per-replan accounting should be run as a bound                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | The credibility of any cross-schedule comparison of adaptation cost                                      |
| 21  | **First-block evaluation convention.** A23 evaluates the first block on the planning anatomy, which is the reference study’s convention and preserves comparability. Evaluating it on the anatomy of the first repeat image gives a conservative bound at the cost of two dose recomputations per arm and no new plans. Whether to report the interval between the two conventions rather than a single value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | The size of the reported margin-reduction benefit                                                        |
| 22  | **Whether to compute the reduced-margin non-adapted diagnostic.** The reduced-margin pCT plan recomputed on the repeat images is not an option of the allocation, but it separates the margin-reduction benefit from the adaptation benefit within the study rather than importing that separation from the reference study’s lung cohort. It costs dose recomputations and no plans                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Nothing in the allocation. It affects only what the manuscript can decompose                             |
...
### 12.1 The four treatments of the free hypofractionated arm

Recorded so that the choice can be made at supervision without reconstructing the argument. None is adopted.
...
| C. Third budget row for photon stereotactic delivery  | On the constraint set               | λ_SBRT as a third curve                                                        | A third sweep axis, and a three-parameter result is harder to present. It is the only treatment that removes the anomaly by pricing a resource rather than by penalising a choice, and it restores A21 to its original form              |
...
## Appendix A. Memo: alternative unit for the photon adaptation budget

Three units were considered for C_XT. Linac minutes were adopted. This memo records the alternative that remains live, so that the choice can be revisited without reconstructing the argument.

| Unit                         | λ_XT means                        | Why not adopted                                                                                                                                                                                                                          |
|------------------------------|-----------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Linac minutes (**adopted**)  | utility per photon machine-minute | Requires a plausible Δτ_XT. Commensurable with λ_PT, so the ratio λ_XT / λ_PT is meaningful and the exchange rate of Section 8.5 is expressible                                                                                          |
| Adaptation events per period | utility per adaptation performed  | Avoids committing to any photon delivery-time quantity, which is the one input the adopted unit needs. Rejected because λ_XT and λ_PT are then in different units and cannot be compared, which removes the investment-allocation result |
| Staff or QA minutes          | utility per physicist-minute      | Closest to the real departmental bottleneck. Rejected because no data exists to anchor it and it would price a resource the extractor does not observe                                                                                   |

**Condition under which the alternative becomes preferable.** If the clinical partners cannot supply a defensible range for Δτ_XT, pricing per adaptation event removes the need for it. The cost is the comparability of the two multipliers, and therefore Section 8.5 in its current form. The formulation is otherwise unchanged: only the units of τ^XT and C_XT differ, and the solver is indifferent.
