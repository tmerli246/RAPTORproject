...
- Dose grid dimensions and masked ROI volumes, to be measured on the first exported case before the storage container is fixed. Together with the crop-versus-mask ratio of X4, the grid ratio of X5, and the comparison between `baseResolution` at the working-grid spacing and at a coarser setting, these are one measurement session on one case.
