...
Version 5.6. Version history is in `CHANGELOG.md`. Project status and open items are in `STATE.md`.
...
- A synthetic-file test for `ingest_struct` and `ingest_plan`, matching what CT and dose already have, deferred at Section 16 given the construction cost of a minimal but genuinely valid RTSTRUCT or RTPLAN file. Not blocked on anything external; a scoping choice to revisit.
- A function that discovers a manifest row's associated plan and structure set from its dose file, closing `manifest.check_row_consistency`'s loop. Depends on knowing whether the manifest's `path` column names a single file or a directory, and whether discovery follows the DICOM reference chain or separate path columns are added: a real export or a decision independent of one, neither available this round. Section 16.
...
**Tested against genuine synthetic files for CT and dose, against wrapper logic only for struct and plan.** Building a minimal but genuinely valid RTSTRUCT or RTPLAN file, with its nested ROI or beam sequences, is materially more involved than CT or dose: a stated scope decision, not a silently smaller test than claimed. The `None`-check logic in all four wrappers is identical in shape; CT and dose exercise it against real parsing, struct and plan against the wrapper alone via `monkeypatch`, substituting `readDicomStruct`/`readDicomPlan` directly rather than constructing a file to trigger the branch. A synthetic-file test for these two, matching CT and dose's, is listed in Section 15 as still open.

**Not built this round.** A function that reads a manifest row's dose file and locates its associated plan and structure set, closing the loop into `manifest.check_row_consistency`, which today takes already-loaded `(dose, plan, struct)` triples and does not say how they are found. The manifest schema's single `path` column per row does not by itself indicate whether it names the dose file alone, with plan and struct discovered by following `dose.referencePlan` and `plan.referencedStructureSetSequence`, or a directory containing all three. Resolving this needs either a real export to inspect or a decision that does not depend on one; neither is available this round, and building the discovery function on a guessed convention risks the same kind of assumption X9 already tracks for RayStation's own conventions. Left open rather than guessed at.
