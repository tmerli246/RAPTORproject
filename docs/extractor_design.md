...
Version 5.8. Version history is in `CHANGELOG.md`. Project status and open items are in `STATE.md`.
...
|---|---|---|
| DICOM ingest | `io.dicomIO`: `readDicomCT`, `readDicomDose`, `readDicomStruct`, `readDicomPlan` | CT, RTDOSE, RTSTRUCT and RTPLAN as typed objects. `readDicomPlan` covers photon IMRT and VMAT and proton PBS, so both modalities are served. **Every one of the last three returns `None`, not an exception, on a configuration it does not recognise**, confirmed by reading `dicomIO` directly on 15 September 2026: `readDicomDose` and `readDicomStruct` on an unsupported pixel format or a missing `SeriesInstanceUID`, `readDicomPlan` on five separate branches. `readDicomCT` has no such branch, but has its own failure mode: derived spacing on a single-slice input is `0/0`, `NaN` rather than an error. `extractor/ingest.py` wraps all four and raises on both patterns, detail in Section 3.4 |
...
**A related pattern, not a default but the same failure to fail loudly.** `readDicomDose`, `readDicomStruct` and `readDicomPlan` return `None`, not an exception, on input they do not recognise: an unsupported pixel format, a missing `SeriesInstanceUID`, five separate unsupported-configuration branches in the plan reader. Confirmed by reading `io.dicomIO` directly on 15 September 2026, and reproduced with a constructed 8-bit dose file rather than only read. `extractor/ingest.py` checks every one of these calls for `None` and raises immediately, naming the file. `readDicomCT` has no such branch but has its own: a single-slice input gives `0/0`, `NaN` spacing, checked separately in Section 16.
...
**Implemented at version 5.4** as `read_manifest` and `check_row_consistency`/`check_manifest` in `extractor/manifest.py`. Two departures from what is written above, both cosmetic rather than substantive.

`fx_scheme` is stored as two CSV columns, `n_fx` and `dose_per_fx_gy`, and reconstructed as the `(n, d)` pair on read: a pair has no unambiguous single-field CSV representation without inventing a delimiter, and the tuple form promised above is what every other part of the codebase sees.

The consistency check verifies three things, each against an attribute confirmed present on the installed OpenTPS's own parsed objects by reading `readDicomDose`, `readDicomPlan` and `readDicomStruct` directly on 14 September 2026, not assumed from the DICOM standard: `dose.referencePlan` equals `plan.sopInstanceUID`; the manifest's own `plan_uid` equals `plan.sopInstanceUID`; and `plan.frameOfReferenceUID` equals `struct.frameOfReferenceUID`. All three raise together, collected into one message, rather than stopping at the first, since a validation pass over a manifest is exactly the situation where every problem at once matters more than one problem per re-run.

**What this does not check, stated precisely because it would be easy to assume it does.** `frameOfReferenceUID` is treated as identifying "the image" for the purpose of matching a row's `source_image_uid`, extending X8. This is a convention this project adopts, not a guarantee DICOM or OpenTPS makes: nothing confirms that RayStation issues a distinct frame of reference for every repeat CT it exports, as opposed to registering repeat images into a shared frame. If it does not, the check would pass rows it should not. Unverified, and joins X9's territory: it closes when a real export can be inspected, not before.
...
with the computing backend and the importing backend behind it. Both are implemented, X2: Morphons since version 5.1, the importing backend since 16 September 2026.

**The imported backend needed a conversion step the design had not anticipated.** `readDicomVectorField` returns a `VectorField3D`, not a `Deformation3D`, confirmed by reading `io.dicomIO` directly on 16 September 2026. The two classes share no interface: `Deformation3D.deformImage(image)` takes an image object and returns one on the fixed grid, the call every backend's caller in this project relies on; `VectorField3D.warp(data)` takes a bare numpy array instead, and neither class is a subclass of the other. Returning the `VectorField3D` as `get_dvf`'s result would have broken `warp_bed` and every other caller the moment the imported backend was used, with an `AttributeError` rather than a design-level warning. The fix is `Deformation3D().initFromDisplacementField(vector_field)`, which reads the field's own `_origin`, `_spacing` and `_angles` and produces a `Deformation3D` that supports `deformImage` exactly as the Morphons path does; verified first against a synthetic `VectorField3D` and then against a synthetic DICOM deformable registration object, built the same way extractor 16's other synthetic DICOM tests are, with a known uniform displacement recovered correctly after conversion and resample.

**What remains untested is narrower than "the imported backend".** The conversion, and the resample onto `working_spacing` both backends share, are implemented and tested; only the read of a real RayStation-exported file is not, since no such file has been inspected. `DIRSettings.imported_path` carries the file to read, required and validated at construction when `backend='imported'`; `moving` is accepted but unused on this path, kept only so a caller's call shape does not change between backends.
...
**Implemented at version 5.4** as `roi_mapping.py`, `load_roi_mapping` and `resolve_dicom_name`, plus `extract_roi_mask_by_canonical_name` in `adapters.py` composing the two with `extract_roi_mask`. The mapping file is a two-column CSV, `canonical_name,dicom_name`, one row per structure, versioned in the repository as stated above; its content hash is computed on load, ready to be carried by the provenance table of Section 13.1, implemented at version 5.6. A header-only file is a valid, intentionally unpopulated mapping, consistent with the mechanism-now-content-later pattern.

Matching is on the normalised form, mechanical only per the rule above, but the value returned is the contour's own un-normalised name, since `getContourByName` and the mask calls it feeds do exact comparison and would not find a name that exists only after normalisation. Two distinct failures, both raises, deliberately worded differently: a canonical name absent from the mapping file is an incomplete mapping, add a row; a canonical name present in the mapping but matching no contour in a given patient's RTSTRUCT is a data problem for that patient, not a mapping problem, and conflating the two would send whoever hits the raise to edit the wrong file.
...
**Implemented at version 5.6** as `extractor/provenance.py`: `ProvenanceRecord` matching the fields above, and `ProvenanceTable`, a keyed, queryable collection. `add` raises on a key already present, on the reasoning that two provenance records for the same primitive most likely means the same quantity was tagged from two call sites unaware of each other, a logic error to surface rather than silently allow; `update` is the separate, explicit spelling for a deliberate re-extraction. `table.query(kind='assumed')` is the enumerability requirement made concrete: the complete, current list of assumed parameters by query, not a second list kept by hand. `write_csv`/`read_csv` persist a table in the same two-column-CSV style as the ROI mapping file, `read_csv` raising on a duplicate key exactly as building the table up with `add` would.

What this module does not and cannot do: decide which quantities are primitives worth tagging in the first place. That judgement, Section 13.2's list, belongs to whoever calls `add`, not to the storage mechanism.
...
| X2 | The deformation field is computed in OpenTPS by default; an imported RayStation field is an alternative backend behind the same interface, `get_dvf(*, moving, fixed, settings, working_spacing)`. The Morphons settings `tryGPU`, `nbProcesses` and `baseResolution` select among different implementations rather than among speeds, are passed explicitly, and all three enter the settings hash. `tryGPU=False` and `nbProcesses=1`; `baseResolution` is set to the working-grid spacing pending the measurement in X5. The imported backend reads via `readDicomVectorField`, `settings.imported_path`, then converts the returned `VectorField3D` into a `Deformation3D` via `initFromDisplacementField`, so both backends return the same type through the same call | Design decision, Section 6.1. Morphons settings read from the source and **benchmarked on 11 September 2026**. The imported backend's conversion path **implemented and tested against a synthetic DICOM deformable registration file on 16 September 2026**; reading a real RayStation export remains untested | Neither source has a known accuracy in the abdomen. If the two disagree materially the choice becomes a reported sensitivity rather than a convention. The GPU path is entered under a bare `except` that logs at INFO, so any failure and not only a missing cupy degrades silently to CPU: passing `tryGPU` explicitly makes the path a choice rather than an outcome. `nbProcesses=1` is no longer a trade against reproducibility, the parallel path being slower at every grid measured, so the `morphonsComplexConvS/D` branch is never taken and one of the three pairs of code paths is removed rather than fixed. `Deformation3D` and `VectorField3D` share no interface, confirmed by reading both classes: `deformImage` takes an image object, `warp` takes a bare array, and neither is a subclass of the other. Without the conversion this function now does, the two backends would not have been interchangeable as the design implied. Whether RayStation exports this object at all, and in what conventions, is X7 and X9 and closes only on a real export |
...
| X8 | Plan identity, that is arm, block, role and scheme, comes from an export manifest; DICOM relations serve as a consistency check, specifically `dose.referencePlan == plan.sopInstanceUID`, the manifest's `plan_uid == plan.sopInstanceUID`, and `plan.frameOfReferenceUID == struct.frameOfReferenceUID`, all confirmed present on the installed OpenTPS's parsed objects. `frameOfReferenceUID` is additionally treated as identifying "the image" for matching against a row's `source_image_uid` | Design decision, Section 4. Implemented and tested against both environments, 14 September 2026 | The manifest is hand-written unless planning is scripted. A displaced row is the failure mode, and the DICOM check is what catches it for the plan-to-structure-set link. The `frameOfReferenceUID`-as-image convention is **unverified**: nothing confirms RayStation issues a distinct frame of reference per repeat CT rather than registering into a shared one, and if it does not, this check would pass rows it should not. Closes with X9, on the first real export |
...
| X11 | A manifest row's `path` names the dose file only; the row's plan and structure set are discovered in the same directory, by matching `SOPInstanceUID` along `dose.referencePlan` and `plan.referencedStructureSetSequence`, rather than the manifest carrying separate path columns for them. Implemented as `manifest.discover_and_load`, tested against genuine synthetic files on both environments, 15 September 2026 | Design decision, Section 16, within this project's control rather than dependent on a RayStation convention: whoever writes the manifest, most likely a script per open decision 26 of the allocator document, also controls where the files it names actually live | If a real export places plan and structure set in a different directory from the dose file, or the export tooling groups files differently, `discover_and_load`'s `search_dir` argument accepts an explicit override, but the default assumption of same-directory siblinghood would need revisiting. Not yet tested against anything but a directory this project's own tests populated |
| X12 | RayStation exports a DICOM deformable registration object (Deformable Spatial Registration Storage) for the DVF between pCT and each rCT_j, in a form `readDicomVectorField` can read: `DeformableRegistrationSequence[0].DeformableRegistrationGridSequence[0]` carrying `ImagePositionPatient`, `GridResolution`, `GridDimensions` and `VectorGridData` | **Unverified.** Distinct from X7, which concerns scenario doses, and from X9's general conventions, which does not name this object specifically. No export has been inspected. The conversion machinery that would consume it, `get_dvf`'s imported backend, is implemented and tested against a synthetic file of this form, Section 6.1 | If RayStation does not export this object, or exports it with different tag population than `readDicomVectorField` expects, `get_dvf(backend='imported')` fails at the read, not at the conversion: the failure is `readDicomVectorField`'s own, an `AttributeError` on a missing sequence, not one this project's code has hardened, since there is nothing yet to harden against |
...
**Closed at version 5.4.** TG-263 resolution and the export manifest are both implemented and tested against both environments this project runs against: `roi_mapping.py`, `extract_roi_mask_by_canonical_name`, `manifest.py`. What remains for each is content and real data, not mechanism, and is listed below rather than treated as still-open design.
...
- The contents of the TG-263 mapping file, which requires a real RTSTRUCT or a template from the clinical partner. The mechanism is complete and tested without it: `config/roi_mapping.csv` ships as a header-only placeholder.
- Whether `frameOfReferenceUID` is a reliable proxy for "the same image" in a RayStation export, that is whether repeat CTs receive distinct frame of reference UIDs rather than sharing one. Unverified, X8, closes with X9 on the first real export.
...
**Closed at version 5.7.** The synthetic-file test for `ingest_struct` and `ingest_plan`, and the manifest-discovery function, both implemented and tested against both environments, Section 16.

**Still open, newly added.** Whether a real RayStation export in fact places a row's plan and structure set alongside its dose file, the assumption `discover_and_load` makes explicit as X11: not yet tested against anything but a directory this project's own tests populated.

## 16. DICOM ingest, implemented at version 5.5

New module, `extractor/ingest.py`: `ingest_ct`, `ingest_dose`, `ingest_struct`, `ingest_plan`, wrapping `io.dicomIO`'s four readers per Section 3.1. 10 tests in `tests/test_ingest.py`, passing against both the public OpenTPS release and the project's own checkout, 15 September 2026.

**The systemic finding.** `readDicomDose`, `readDicomStruct` and `readDicomPlan` all return `None`, not an exception, on input they do not recognise. This is not one function's quirk: it is the consistent pattern across three of the four readers, confirmed by reading `dicomIO` directly rather than found by chance in one. `readDicomDose` on an unsupported `BitsStored`/`PixelRepresentation` combination, only 16- and 32-bit signed and unsigned are recognised. `readDicomStruct` on a file with no `SeriesInstanceUID`. `readDicomPlan` on five separate branches: an unsupported photon or ion radiation type, a proton scan mode other than `'MODULATED'`, an unsupported ion scan mode, or an unrecognised `SOPClassUID`, and which of the five fired is not recoverable from the return value alone. A caller that does not check for `None` gets a confusing `AttributeError` several calls downstream, on whatever first touches the missing object, rather than a signal at the point the file actually failed. `ingest.py` checks every call and raises immediately, naming the file and, where the source makes it identifiable, the reason.

**`readDicomCT` has no `None` branch, confirmed by the same read, and a different failure mode instead.** It derives z-spacing as `(last - first) / (n - 1)` over the slices given. With zero files this indexes an empty array, `IndexError`, uninformative about what was actually wrong. With exactly one file it is `0/0`: not an exception, a `NaN` spacing that silently corrupts every downstream geometry calculation touching it, since nothing else in the pipeline checks a spacing for being finite before using it. `ingest_ct` rejects both before the call, with a message naming which case fired.

**Tested against genuine synthetic files for all four readers, since 15 September 2026.** Struct and plan were tested at the wrapper-logic level only when this section was first written, `monkeypatch` substituting for a minimal valid RTSTRUCT or RTPLAN file whose construction cost seemed disproportionate on its own. Building the manifest's discovery function, below, needed exactly that construction anyway, in `tests/dicom_builders.py`, shared across the files that need it rather than duplicated: a minimal RT Ion Plan, one beam, one layer, a handful of spots, and a minimal RTSTRUCT, both confirmed to parse on the first attempt once the required tag set was read from the source rather than guessed. The wrapper-logic tests are kept alongside the new file-based ones, not replaced: a valid file never exercises the `None` branch, so that behaviour still needs its own coverage regardless of how good the fixtures are.

**The export manifest's discovery function, closing the gap Section 4 left open, is implemented as `manifest.discover_and_load`.** A design decision this project controls, not a RayStation convention to wait for: a manifest row's `path` names the dose file, and every file that row needs is discovered alongside it by following `dose.referencePlan` and `plan.referencedStructureSetSequence` to matching `SOPInstanceUID`s in the same directory, rather than by adding path columns the schema does not have. `manifest.py`'s stated "no OpenTPS import" property no longer holds for this one function, which calls `ingest.ingest_dose`/`ingest_plan`/`ingest_struct` to actually load what it finds; stated in that module's own docstring rather than left for a reader to notice the mismatch.

**A secondary finding from building the end-to-end test, documented in evaluator design 11.6, repeated here since it is this module's own reader.** `readDicomCT`'s `.transpose(1, 0, 2)`, converting DICOM's (Rows, Columns, Slices) to OpenTPS's internal order, was invisible to every test in this section, all built on spatially uniform phantoms, transpose-invariant by construction. A spatially varying phantom is not, and `tests/dicom_builders.py`'s `write_ct_series` says so in its own docstring rather than silently producing an array that does not round-trip to what a naive reading of "write Rows as the first axis" would suggest. Not a defect in `ingest_ct`: a caller supplying a correctly DICOM-oriented array would not see it. No code change follows from this in this module, since `ingest.py` only reads; the finding is for whoever writes DICOM, in a test fixture or eventually for real.
