...
routes on 14 September 2026 gives EQD2(5 Gy, one fraction, alpha/beta 3) =
8.00 Gy, not the 10.00 Gy the design document states, and consequently a
row-2 result of 4.40 Gy rather than 5.40 Gy. Row 1 (3.60 Gy) checks out
exactly. Flagged to Tommaso rather than silently corrected or silently
trusted; this file tests everything else, using numbers this test suite
computes and owns.
"""
...
class TestGeudFromDvh:
    ...
    def test_uniform_dose_gives_geud_equal_to_dose_at_any_a(self):
        eqd2 = _uniform_dose(60.0)
        mask = _full_mask()
        dvh = reduce_to_dvh(eqd2, mask)

        for a in (1.0, 1 / 0.09, 5.0, 0.3):
            geud = geud_from_dvh(dvh, a)
            assert geud == pytest.approx(60.0, rel=0.02), a

    def test_two_value_case_matches_hand_calculation(self):
        """Half the ROI at 40 Gy, half at 80 Gy, a = 2.
    ...
    def test_small_volume_parameter_approaches_max_dose(self):
        """As a -> large (n -> small, a serial-like organ), gEUD should
        approach the maximum dose present, the power-mean limit. Not an
        exact equality test, a direction-and-bound check: gEUD must lie
        between the mean and the max, and move toward the max as a grows.
        """
        array = np.empty(GRID, dtype=np.float32)
        half = GRID[2] // 2
        array[:, :, :half] = 40.0
        ...
        geud_low_a = geud_from_dvh(dvh, a=1.0)     # equals the mean, 60.0
        geud_high_a = geud_from_dvh(dvh, a=20.0)   # should approach 80.0

        assert geud_low_a == pytest.approx(60.0, rel=0.02)
        assert geud_high_a > geud_low_a
        assert geud_high_a < 80.0
        assert geud_high_a > 75.0   # should be close to the max by a=20
