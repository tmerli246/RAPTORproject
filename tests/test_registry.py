...
def test_registry_lkb_matches_ntcp_module():
    dose = RNG.uniform(20.0, 70.0, 800)
    old = MODELS['rectum_bleeding_g2']
    new = REGISTRY['rectum_bleeding_g2']
    direct = lkb_ntcp(dose, 28, old)
    via_registry = evaluate(new, eqd2_dose = eqd2(dose, 28, new.alpha_beta))
    assert via_registry == pytest.approx(direct, abs = 1e-12)
